...
type Props = {
  ...
  // Legacy props para compatibilidad
  alergiasText?: string
...
export default function AddPatientStepSalud({
  ...
  // Estado para nueva alergia
  const [newAllergyName, setNewAllergyName] = React.useState('')
  const [newAllergySeverity, setNewAllergySeverity] = React.useState<AllergySeverity>('moderada')
  ...
  return (
    <div className='left-[18.375rem] top-[10rem] absolute inline-flex flex-col justify-start items-start gap-6 w-[31.5rem] h-[43.25rem] overflow-y-auto overflow-x-clip pr-2 pb-2 scrollbar-hide'>
      <div className='inline-flex flex-col gap-3 w-full'>
            ...
            {alergias.map((allergy) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <MD3Icon name='CloseRounded' size='sm' />
                  </button>
                </div>
              )
            })}
        ...
        {/* Formulario para añadir nueva alergia */}
        ...
        {/* Alergias graves/extremas muestran alerta */}
        {alergias.some(a => a.severity === 'grave' || a.severity === 'extrema') && (
          <div className='flex items-center gap-2 p-3 rounded-lg bg-[var(--color-error-50)] border border-[var(--color-error-200)]'>
            <MD3Icon name='WarningRounded' size='md' className='text-[var(--color-error-600)]' />
            <span className='text-body-sm text-[var(--color-error-700)]'>
              Este paciente tiene alergias graves o extremas. Se mostrará una alerta visible en su ficha.
            </span>
          </div>
        )}
      </div>
